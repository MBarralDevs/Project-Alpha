 ...
 */
import { createPublicClient, createWalletClient, http, parseEther, formatEther, zeroHash, namehash, type Address, type Hex } from "viem";
...
if (!RESOLVER || !/^0x[0-9a-fA-F]{40}$/.test(RESOLVER)) fail("ENS_RESOLVER_ADDRESS missing/malformed");
...
const registerAbi = [{
  type: "function", name: "register", stateMutability: "payable", outputs: [],
...
const registry = [{ type: "function", name: "resolver", stateMutability: "view", inputs: [{ type: "bytes32" }], outputs: [{ type: "address" }] },
  { type: "function", name: "owner", stateMutability: "view", inputs: [{ type: "bytes32" }], outputs: [{ type: "address" }] }] as const;

async function main() {
  const reg = { label, owner: account.address, duration: DURATION, secret: zeroHash,
    resolver: RESOLVER!, data: [] as Hex[], reverseRecord: 0, referrer: zeroHash };
  ...
  const { request } = await client.simulateContract({
    address: CONTROLLER, abi: registerAbi, functionName: "register", args: [reg], account, value: VALUE,
  });
  ...
  const [owner, resolver] = await Promise.all([
    client.readContract({ address: CLASSIC_REGISTRY, abi: registry, functionName: "owner", args: [node] }).catch(() => "0x?"),
    client.readContract({ address: CLASSIC_REGISTRY, abi: registry, functionName: "resolver", args: [node] }).catch(() => "0x?"),
  ]);
  console.log(`  classic registry owner(${NAME}):    ${owner}`);
  console.log(`  classic registry resolver(${NAME}): ${resolver}`);
  console.log(resolver && (resolver as string).toLowerCase() === RESOLVER!.toLowerCase()
